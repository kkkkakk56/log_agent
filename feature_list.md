...
| `TASK-018` | `ready` | 知识库 Park 与知识笔记：用户可以进入独立知识库区域，创建学习库，并在每个知识库下持续追加知识记录 | `cd journal-agent && npm run build && npx cap sync ios` |
...
| `TASK-020` | `planned` | 项目实验库里的项目记录：按项目聚合目标、进展、决策、问题和阶段性复盘 | `cd journal-agent && npm run build` |
...
在项目实验库 Park 下，按项目聚合目标、进展、决策、问题和阶段性复盘，适合长期项目、学习计划、开发任务或个人事务管理。

验收标准：

- 用户可以创建一个项目记录空间。
- 项目包含目标、状态、里程碑、相关链接和标签。
- 项目下可以追加进展记录、决策记录、问题记录和复盘记录。
- 项目列表展示当前状态、最近更新和记录数量。
- 搜索可以从项目维度找到相关记录。
...
当前优先实现 `TASK-018 / F022：知识库 Park 与知识笔记`。Park 框架已经完成，下一步是真正让知识库区支持创建学习库，并在每个学习库下追加知识记录。

项目实验库和计划区先作为后续 Park 保留在路线图里。
