...
| `TASK-008` | `done` | 搜索体验优化：高亮关键词、清空查询、显示命中片段 | `cd journal-agent && npm run build` |
| `TASK-036` | `done` | 记录重要性标记：任意记录都可以用小巧的 `⚑` flag 标记为重要，便于快速识别 | `cd journal-agent && npm run build && npx cap sync ios` |
...
| `TASK-035` | `done` | 密库 Park：新增本地加密的账号密码管理区，用自定义密码解锁，并支持一键复制账号和密码 | `cd journal-agent && npm run build && npx cap sync ios` |
| `TASK-037` | `done` | 笔记与做记置顶：知识库笔记和做记记录支持置顶显示，并限制每个仓库或项目最多 5 条 | `cd journal-agent && npm run build && npx cap sync ios` |
| `TASK-038` | `done` | 卡片三点操作菜单：每张记录卡片都提供 `⋯` 入口，直接集成常用操作，不必先进入详情 | `cd journal-agent && npm run build && npx cap sync ios` |
| `TASK-039` | `done` | 图片导入：心记、知识笔记和做记记录支持从系统相册或文件导入图片，并在记录中展示 | `cd journal-agent && npm run build && npx cap sync ios` |
| `TASK-040` | `done` | 思记 Park：新增轻量问题花园，用“问题 -> 回响”组织持续性值得思考的问题 | `cd journal-agent && npm run build` |
...
- 用户可以为心记、知识笔记和做记记录设置一次性定时提醒；同一条记录或同一句话支持设置多个不同时间的提醒，并且可以同时生效。
...
- 当前交互保持不变，用户可以像现在一样重复创建提醒；底层现在会为同一条记录保留多条未来提醒，并在 App 启动或回到前台时重新同步给系统通知，避免后设置的提醒把先前提醒挤掉。
...
搜索结果现在支持关键词高亮、清空查询和命中摘要片段，长内容也能更快看到相关上下文。

验收标准：

- 命中的关键词会在标题、正文摘要和匹配标签里被明显高亮。
- 搜索框提供一键清空按钮，并支持快速回到空搜索状态。
- 长日志优先显示围绕命中位置的摘要片段，展开后再看全文。
- 保持轻量实现，不引入重型搜索库。
...
### F037：密库 Park 与本地加密账号管理

状态：`done`

新增一个独立的“密库” Park，用来保存项目、网站或服务的账号密码卡片。当前版本先采用你允许的降级方案：进入这个 Park 需要输入自定义解锁密码；数据会在本地用该密码派生出的密钥加密保存，离开 Park 或切到后台后会自动重新上锁。

验收标准：

- 顶层导航新增一个独立的密库 Park，不和心记、笔记、做记内容混在一起。
- 首次进入密库时，需要先设置一组自定义解锁密码；当前版本先不强依赖 Face ID。
- 已设置密码后，重新进入密库前必须先解锁，未解锁时不能查看已有账号密码卡片内容。
- 密库里的每张卡片至少包含项目 / 服务名称、账号、密码，并支持可选入口地址和备注。
- 每张卡片里的账号和密码分别提供独立复制按钮，可以一键复制，不需要手动选中文本。
- 密码默认遮挡显示，但可以手动显示或隐藏，避免列表里长期明文暴露。
- 用户可以新增、编辑、删除账号密码卡片。
- 切换到其他 Park 或 App 进入后台后，密库会自动上锁。

实现说明：

- 自定义密码不直接明文保存；会先通过 `PBKDF2 + AES-GCM` 派生本地加密密钥，再保存密库内容。
- 当前版本把“Face ID 进入 Park”的要求先用“自定义解锁密码”落地，避免额外原生依赖阻塞开发；后续如果接入 Face ID，可以复用现有门禁流程。
- 复制功能优先走 `Clipboard API`，失败时退回到基于隐藏文本框的复制方案，尽量兼容浏览器和 WebView。
...
知识库笔记和做记记录现在都支持置顶，重点内容会固定在列表前面；每个知识库或做记项目最多保留 5 条置顶，避免列表被长期占满。

验收标准：

- 知识库笔记支持置顶和取消置顶，并在笔记列表中优先显示。
- 做记记录支持置顶和取消置顶，并在记录列表中优先显示。
- 置顶状态会持久化保存，刷新或重启 App 后不会丢失。
- 已置顶内容在列表和详情里都有明确标记。
- 每个知识库最多置顶 5 条笔记，每个做记项目最多置顶 5 条记录。
- 达到上限后，继续置顶会给出清晰提示。

实现说明：

- 置顶状态使用独立的 `pinnedAt` 元数据保存，不改写正文、标题和更新时间。
- 知识库与做记列表统一按“已置顶优先，再按更新时间”展示，方便把重点内容固定在前面。
- 详情页提供置顶 / 取消置顶入口，列表头部会显示当前已置顶数量，帮助控制在上限内。
...
现在心记、知识笔记和做记记录都支持 `flag`。第一版采用小巧的 `⚑` 符号来表达“这条很重要，之后要更容易回看”，并把状态独立保存，不改写正文或标题。

验收标准：

- 当前支持的心记、知识笔记和做记记录都支持标记或取消重要。
- 重要记录会在列表和详情里展示统一的小符号 `⚑`。
- 重要标记不会改写原始标题和正文，只作为独立元数据保存。
- 关闭并重新打开 App 后，重要标记状态仍然保留。
- 第一版先做二元重要标记，不扩展为多级优先级。

实现说明：

- 重要状态使用独立的 `flaggedAt` 元数据保存，不改写正文、标题和更新时间。
- 心记在时间线、搜索结果、日历选中日和已完成分组里都能显示 `⚑`，并支持一键 `Flag / 取消 Flag`。
- 知识笔记和做记记录在列表与详情页都能显示 `⚑`，详情页提供 `Flag` 开关。
- Agent 读取上下文时也会保留这个标记，后续让 Agent 帮你回看重点内容时可以感知到哪些记录被你标过。
...
为心记、知识笔记和做记记录卡片增加统一的 `⋯` 操作入口。用户点卡片时继续看内容、展开或选中；用户点 `⋯` 时在当前卡片旁原地展开轻量菜单去执行操作，不必先进入详情页。

验收标准：

- 心记、知识笔记和做记记录卡片都存在统一的 `⋯` 操作入口。
- 点击 `⋯` 后会在当前卡片旁原地打开轻量菜单，而不是弹出会影响整个屏幕的动作面板。
- 心记卡片可以直接执行 `Flag`、提醒、待办、编辑、删除。
- 知识笔记卡片可以直接执行 `Flag`、置顶、提醒、编辑、删除。
- 做记记录卡片可以直接执行 `Flag`、置顶、待办、提醒、编辑、删除。
- 卡片主体点击行为继续保留，不因为引入 `⋯` 菜单而失效。

实现意图：

- 看内容和做操作分开：看内容点卡片，做操作点 `⋯`。
- 心记卡片原来底部散开的操作按钮会收口进 `⋯`，避免界面太吵。
- 三类记录卡片尽量使用同一套原地菜单，减少“这类卡片要先进详情、那类不用”的记忆负担。
...
支持在心记、知识笔记和做记记录里导入本地图片。第一版走本地优先方案：从系统相册或文件选择图片后，客户端会自动压缩，再把图片跟记录一起保存到本地。

验收标准：

- 心记、知识笔记和做记记录在新增与编辑时都可以导入图片。
- 导入后的图片会跟随记录一起持久化保存，刷新或重启 App 后仍能看到。
- 记录卡片在展开查看时可以直接看到图片，并支持点开预览大图。
- 用户在保存前可以移除某张已选图片，避免误传。
- 第一版为避免本地存储被快速占满，单条记录最多保留 4 张图片，并在导入时自动压缩。

实现意图：

- 先把“图文记录”这件事做通，让心记、知识笔记和做记都能承载截图、照片、白板图或资料图。
- 第一版不引入新的云端或文件系统依赖，优先复用当前本地存储架构，降低实现复杂度。
- 通过自动压缩和单条上限控制，把 `localStorage` 方案还能承受的空间窗口尽量拉大。
...
在心记首页头部新增一块紧凑天气提示，放在标题右侧、`今日` 计数左侧，并使用免费天气 API + Vue/CSS 做出接近 Apple Weather 气质的展示。

验收标准：

- 仅在心记首页头部显示天气卡，不影响其他 Park 的顶部布局。
- 数据层使用免费天气 API，首版不要求用户配置 API Key。
- 卡片支持显示当前温度、天气概况和最高/最低温，以紧凑条状组件呈现。
- 卡片视觉采用渐变背景和轻玻璃质感，但尺寸保持克制，不抢标题区视觉。
- iOS 端天气图标通过 Capacitor 桥接调用原生 SF Symbols 渲染。
- 若定位失败，界面仍能回退到最近一次位置或默认城市，避免首页完全空白。

实现意图：

- 把“今天外部环境怎么样”直接放进首页头部，让用户打开心记时先看到当下，但不让天气块压过标题本身。
- 数据层优先选无 Key 的 `Open-Meteo`，降低接入成本，也避免把第三方天气密钥塞进客户端。
- 视觉层延续 F013 的 iOS 手记气质，但保持在“inspired”范围内，不使用 Apple 专有素材或截图。
- 原生 SF Symbols 通过 Capacitor 插件输出成位图给前端显示，保证 iOS 上的天气图标仍然是原生系统图形。
...
新增一个轻量、安静的“思记” Park，用来放那些还没有答案、但值得反复遇见的问题。第一版不做标签、状态、分类或项目管理，只保留最小结构：`问题 -> 回响`。

验收标准：

- 顶层导航出现独立的“思记” Park。
- 思记首页只展示问题列表，每个问题显示问题本身、最近回响摘要、回响数量和最近更新时间。
- 用户可以通过底部中间 `+` 新增问题，只需要填写问题本身，为什么值得想为可选。
- 点进问题后进入安静的回响流，按时间展示这个问题下的思考片段。
- 问题详情页的底部中间 `+` 用来追加回响。
- 数据本地持久化保存，刷新或重启后仍能看到问题和回响。

实现意图：

- 把思记定位成低压、长期、非任务化的问题空间，避免做成另一个知识库或 Todo。
- 第一版刻意不引入状态、标签、分类和复杂操作，让用户只需要放下问题、偶尔追加回响。
- 视觉上保持更干净安静：少边框、少统计、少控制，让问题和回响本身成为主体。
...
知识库抽屉导航、做记 Park 项目记录、图片导入、Agent 代写三记、系统提醒、日记未来日期编辑、Todo 标记、记录重要性标记和 Agent 浮钮位置可调都已经可用。下一步可以回到 `TASK-005 / F005：草稿自动保存`，或者继续补 `F023：实验进度记录`。
